...
const processor: Processor = async (
  job: Job<{ entityType: 'feedback' | 'file'; entityId: string }>,
) => {
  logger.log(
  ...
  return await NotificationsService.processNotificationStatic(
    jobData,
    mockConfigService,
  );
};
...
worker.on('completed', (job) => {
  logger.log(
    `✅ Job ${job.id} completed for ${job.data.entityType} ${job.data.entityId}`,
  );
});
...
worker.on('failed', (job, error) => {
  if (job) {
    logger.error(
      `❌ Job ${job.id} failed for ${job.data.entityType} ${job.data.entityId}: ${error.message}`,
    );
  } else {
...
worker.on('active', (job) => {
  logger.log(
    `🔄 Started processing job ${job.id} for ${job.data.entityType} ${job.data.entityId}`,
  );
});
