...
export class NotificationsService {
  ...
  async enqueueNotification(entityType: 'feedback' | 'file', entityId: string): Promise<void> {
    if (!entityType || !entityId) {
      this.logger.error(`Invalid job data: entityType=${entityType}, entityId=${entityId}`);
      throw new Error('Invalid job data: entityType and entityId are required');
    }
    const job = await this.notificationsQueue.add('notify', { entityType, entityId });
    this.logger.log(`📤 Enqueued notification job ${job.id} for ${entityType} ${entityId}`);
  }
  ...
  static async processNotificationStatic(
    ...
    logger.log(`🚀 Processing notification for ${data.entityType} ${data.entityId}`);
...
try {
  ...
  logger.log(`✅ Sent Discord webhook for ${data.entityType} ${data.entityId}: ${response.status}`);
  ...
  const errorMessage = error.response
    ...
    : error.message;
  logger.error(`❌ Failed to send Discord webhook for ${data.entityType} ${data.entityId}: ${errorMessage}`);
  throw new Error(errorMessage);
}
